...
<body>
	...
	if (isset($_SESSION['reg_character'])) {
		...
		$user_details_query = mysqli_query($con, "SELECT * FROM entertainment WHERE city='$city'");
	...
	<form action="commentsa.php?post_id=<?php echo $post_id; ?>" id="comment_form" name="postComment<?php echo $post_id; ?>" method="POST">
		<textarea name="post_body" style="    position:relative;
    margin-top: 1%;"></textarea>
		<input type="submit" name="postComment<?php echo $post_id; ?>" value="Post" style="background-color: #5ba5d7;">
	</form>

	<!-- Load comments -->
		<?php 
		$get_comments=mysqli_query($con,"SELECT * FROM comments WHERE  posta='$post_id' ORDER BY sno ASC");
